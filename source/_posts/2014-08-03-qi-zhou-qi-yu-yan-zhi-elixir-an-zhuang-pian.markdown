...
comments: true
categories: [erlang, elixir, 七周七语言]
---
...
以上测试包含了整数相加，整合和浮点数相加，打印输出，字符串拼接，模块定义和方法的定义/调用。值得注意的是：  

1. 在elixir中的字符串拼接使用的是 `<>` 而不是`+`。  
2. 方法的定义必须在`module`中，不然会报`(ArgumentError) cannot invoke def/2 outside module`错误。  
